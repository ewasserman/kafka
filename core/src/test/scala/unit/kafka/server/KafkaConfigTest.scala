  ...
  def testFromPropsInvalid() {
    ...
    KafkaConfig.configNames().foreach(name => {
      name match {
        ...
        case KafkaConfig.LogCleanerMinCompactionLagMessagesProp => assertPropertyInvalid(getBaseProperties(), name, "not_a_number")
        case KafkaConfig.LogCleanerMinCompactionLagBytesProp => assertPropertyInvalid(getBaseProperties(), name, "not_a_number")
